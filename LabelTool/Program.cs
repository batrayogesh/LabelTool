...
namespace SplitLabelFiles
{
    ...
    {
        // Required Inputs
        const string currentLabelFilePath = @"E:\git\ApplicationSuite\Source\Metadata\SubscriptionBilling\SubscriptionBilling\AxLabelFile\SubBill_en-US.xml";
        const string newLabelFilePath = @"E:\git\ApplicationSuite\Source\Metadata\SubscriptionBilling\SubscriptionBilling\AxLabelFile\SubBill2_en-US.label.xml";
        const string actualNewLableFilePath = @"E:\git\ApplicationSuite\Source\Metadata\SubscriptionBilling\SubscriptionBilling\AxLabelFile\LabelResources\en-US\SubBill2_en-US.label.txt";
        const string AOTpath = @"E:\git\ApplicationSuite\Source\Metadata\SubscriptionBilling\SubscriptionBilling\";

        // Optional input: provide a team name if it is different from the owner of the new label file
        const string TeamName = "PM-Nano";
         ...
         */
            ...
            foreach (Label l in labelFileLabels)
            {
                ...
                if (usedLabel == null)
                {
                    l.labelDescription = l.labelDescription + " Label is not used.";
                ...
                else
                {
                    ...
                    foreach (Label currentUsedLabel in labels)
                    {
                        if (l.labelId.ToUpper().Equals(currentUsedLabel.labelId.ToUpper()))
                        {
                            ...
                            //parse location
                            string name = currentUsedLabel.fileUsedIn.Replace(AOTpath, "");
                    ...
                    //l.labelDescription = l.labelDescription + " Used in " + descriptionUsage.ToString();
                    l.labelDescription = l.labelDescription + " Used in " + String.Join(", ", stringSet);
        ...
        public static List<Label> ParseLabelFile(string filepath, out string actualLabelFilePath, out string prefix)
        {
            ...
            String formattedFile = formatTheInputLabelFile(actualLabelFilePath);
            
            
            string[] text = System.IO.File.ReadAllLines(formattedFile);
        ...
        private static string formatTheInputLabelFile(string actualLabelFilePath)
        {
            string formattedLabelFilePath = AddSuffix(actualLabelFilePath, "_formatted");

            string[] lines = File.ReadAllLines(actualLabelFilePath);
            ...
            for (int i = 0; i < lines.Length; i++) {
                if (colonExpected) {
                    if (lines[i].Contains(";"))
                    {
                        if (lines[i].Contains("Used in"))
                        {
                            formattedLines.AppendLine(lines[i].Substring(0, lines[i].IndexOf("Used in")-1));
                        }
                        else
                        {
                            formattedLines.AppendLine(lines[i]);
                        }
                        colonExpected = false;
                    }
                    else {
            ...
            using (System.IO.StreamWriter file = new System.IO.StreamWriter(formattedLabelFilePath))
            {
                file.Write(formattedLines.ToString());
            }
        ...
        static List<Label> getLabelsWithOwners(FileInfo[] files, List<Label> labelFileLabels)
        {
            ...
            foreach (FileInfo file in files)
            {
                ...
                for (int i = 0; i >= 0 && i < text.Length; i++)
                {
                    ...
                    //Some labels in reports come witha lot of extra text. This shortens it to just the label
                    if (labelUsed.Contains("&"))
                    {
                        int subIndex = labelUsed.IndexOf("&");
                        labelUsed = labelUsed.Substring(0, subIndex);
                    }
        ...
        public static FileInfo[] getFilesWithUsage(string team, string filePathForCache, bool record = false)
        {
            if (record)
            {
                recordFilesWithUsage(team, filePathForCache); //Filter out the files which has no labels at all
            }
        ...
        static string parseLabel(string text, string labelPrefix, int labelLocation)
        {
            ...
            while (!endOfLabelFound)
            {
                ...
                if (!Char.IsLetterOrDigit(characterAtLocation))
                {
